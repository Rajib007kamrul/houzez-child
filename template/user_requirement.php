<?php
/**
 * Template Name: User Dashboard Client Requirment
 * Created by PhpStorm.
 * User: rokibul
 * Date: 15/10/15
 * Time: 3:33 PM
...
$dashboard_add_listing = houzez_get_template_link_2('template/user_requirment_submit.php');

get_header();

if( isset( $_GET['request_status'] ) && $_GET['request_status'] == 'publish' ) {
    $qry_status = 'publish';

} elseif( isset( $_GET['request_status'] ) && $_GET['request_status'] == 'pending' ) {
    $qry_status = 'pending';

} elseif( isset( $_GET['request_status'] ) && $_GET['request_status'] == 'expired' ) {
    $qry_status = 'expired';
} elseif( isset( $_GET['request_status'] ) && $_GET['request_status'] == 'draft' ) {
    $qry_status = 'draft';
} elseif( isset( $_GET['request_status'] ) && $_GET['request_status'] == 'on_hold' ) {
    $qry_status = 'on_hold';
} else {
    $qry_status = 'any';
}
    ...
    <div class="dashboard-header-wrap">
        <div class="d-flex align-items-center">
            <div class="dashboard-header-left flex-grow-1">
                <h1><?php echo 'Requirements'; ?></h1>         
            </div><!-- dashboard-header-left -->

            <?php if(!empty($dashboard_add_listing)) { ?>
            <div class="dashboard-header-right">
                <a class="btn btn-primary" href="<?php echo esc_url($dashboard_add_listing); ?>"><?php echo 'Create a Requirements'; ?></a>
            ...
            if( $prop_qry->have_posts() ): ?>
                ...
                <tbody>
                    ...
                    while ($prop_qry->have_posts()): $prop_qry->the_post();
                        ...
                        get_template_part('template-parts/client-requirements/requirement-item');
