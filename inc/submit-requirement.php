...
add_filter('houzez_submit_requirement_listing', 'houzez_submit_requirement_listing');

if( !function_exists('houzez_submit_requirement_listing') ) {
    function houzez_submit_requirement_listing( $new_property ) {

        $userID = get_current_user_id();
        $listings_admin_approved = houzez_option('requirement_listings_admin_approved');
        $edit_listings_admin_approved = houzez_option('requirement_edit_listings_admin_approved');
        ...
        if( $submission_action == 'add_requirement' ) {
             ...
             */
            $new_property = apply_filters( 'houzez_before_submit_requirement', $new_property );
        ...
        } else if( $submission_action == 'update_requirement' ) {
             ...
             */
            $new_property = apply_filters( 'houzez_before_update_requirement', $new_property );
        ...
        if( $prop_id > 0 ) {
            ...
            if( ( isset($_POST['lat']) && !empty($_POST['lat']) ) && (  isset($_POST['lng']) && !empty($_POST['lng'])  ) ) {
                ...
                if( isset( $_POST['prop_google_street_view'] ) ) {
            ...
            if( $submission_action == 'add_requirement' ) {
                do_action( 'houzez_after_reuirement_submit', $prop_id );

                if( houzez_option('add_new_requirement') == 1 ) {
                    houzez_webhook_post( $_POST, 'houzez_add_new_reuirement' );
                }

            } else if ( $submission_action == 'update_requirement' ) {
                do_action( 'houzez_after_requirement_update', $prop_id );

                if( houzez_option('add_new_property') == 1 ) {
                    houzez_webhook_post( $_POST, 'houzez_update_requirement' );
                }
            }
